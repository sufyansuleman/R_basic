...
5.  **Save the R Markdown file**:
    ...
    -   In the dialog box that appears, navigate to the `learning R` directory that you created.
...
-   In this module, we will work in a R Markdown file. We won't be creating an R Script at this stage; that will happen later in Module 3 when we will learn data analysis.
...
::: {.callout-note}
In Module 2, we will explore how to name files and directories and delve into project management best practices.
:::
...
::: {.callout-note}
For the time being, please use the class function to understand data types. We will learn more about details of the function/s later.
:::
...
1. Recall that we created the objects num1, char1, and log1 earlier in Section 2.3. If they are visible in your environment pane, employ the class function to ascertain the type of data they contain. If they are not visible, recreate them by rerunning the code from sections 2.3.1 to 2.3.3.

2. When executing code in R Markdown, you can either click the green arrow button at the top right corner of the chunk to run the entire code chunk or position your cursor on the specific line of code you wish to run and press Ctrl + Enter, which will execute only that particular line of code.
:::
...
3. perform this inside the workbook file
...
1. A numeric variable named `numeric_variable` is created and assigned a value of 10. A basic arithmetic operation (addition) is then performed on this variable.
2. A character variable named `name_variable` is created and assigned the value "Your Name".
3. A logical variable named `comparison_variable` is created by comparing two numbers using the greater than (`>`) operator.
:::
