...
#| output: asis
library(dplyr)
add_link <- function(text, link) {
  colour <- "#138d75"
  added_link <- paste0("<i class='fa-solid fa-", "' style='color:", colour, "'></i>&nbsp;&nbsp;<a href=\"", link, "\">", text, "</a>")
  if (link == "") {
    added_link <- paste0("<i class='fa-solid fa-", "' style='color:", colour, "'></i>&nbsp;&nbsp;", text)
  }
...
schedule <- here::here("programme/schedule.csv") %>%
  ...
  mutate(
    `Session topic` = list(`Session topic`, link) %>%
